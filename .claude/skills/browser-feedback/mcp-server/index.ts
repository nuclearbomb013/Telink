...
class BrowserManager {
  ...
  private hookPage(page: Page) {
    ...
    page.on("response", async (res) => {
      ...
      const startTime = (req as { _startTime?: number })._startTime || Date.now();
      // Find matching entry and update it
      const entry = this.networkRequests.find(
        (e) => e.url === req.url() && e.status === 0
      );
      if (entry) {
  ...
  async reload() {
    if (!this.page || !this.currentUrl) throw new Error("No page to reload. Navigate first.");
    this.consoleLogs = [];
    this.networkRequests = [];
  ...
  async screenshot(fullPage: boolean, selector?: string) {
    const page = await this.ensurePage();
    const ts = Date.now();
...
server.tool(
  "browser_navigate",
  "Navigate the browser to a URL (e.g. http://localhost:5173). Also launches the browser if not already running. Clears previous console/network logs.",
...
server.tool(
  "browser_reload",
  "Reload the current page. Clears previous console/network logs. Useful after making code changes.",
  {},
  async () => {
    try {
      await bm.reload();
...
server.tool(
  "browser_screenshot",
  "Take a screenshot of the current page (full page or viewport). Returns the file path; use the Read tool to view the image.",
  {
...
server.tool(
  "browser_get_console",
  "Get collected browser console logs. Filters by type (error, warning, log, info, debug).",
  {
...
server.tool(
  "browser_get_network",
  "Get collected network requests. Can filter by status code or resource type.",
  {
  ...
  async ({ statusFilter, resourceType, limit }) => {
    ...
    if (statusFilter === "ok") items = items.filter((r) => r.status >= 200 && r.status < 400);
    else if (statusFilter === "error") items = items.filter((r) => r.status >= 400 || r.status === 0);
    else if (statusFilter === "pending") items = items.filter((r) => r.status === 0);
...
server.tool(
  "browser_get_dom",
  "Inspect an element by CSS selector. Returns existence, text content, attribute values, and bounding box.",
  {
...
server.tool(
  "browser_evaluate",
  "Execute arbitrary JavaScript in the browser page context and return the result. Use for reading runtime state (React state, store values, computed styles, etc.).",
  {
...
server.tool(
  "browser_get_metrics",
  "Collect page performance metrics including navigation timing, paint timing, and layout stability.",
  {},
  async () => {
    try {
      const page = await bm.ensurePage();
      const metrics = await page.evaluate(() => {
        ...
        const paintTimings: Record<string, number> = {};
        ...
        return {
          ...
          memory: (performance as unknown as { memory?: { usedJSHeapSize: number; totalJSHeapSize: number } } | undefined)?.memory
            ? {
                usedJsHeapMB: Math.round((performance as unknown as { memory: { usedJSHeapSize: number } }).memory.usedJSHeapSize / 1048576 * 100) / 100,
              }
            : null,
        };
      });
...
server.tool(
  "browser_check_a11y",
  "Run basic accessibility checks on the current page. Checks for: missing alt text, empty buttons, low contrast warnings, missing form labels, ARIA issues. For a full audit, use browser_evaluate with axe-core.",
  {},
  async () => {
    try {
      const page = await bm.ensurePage();
